...
lazy val copyFastJsTask = Def.task {
  (client / Compile / fastLinkJS).value
  copyFiles(
    from = (client / Compile / fastLinkJS / scalaJSLinkerOutputDirectory).value,
...
lazy val copyFullJsTask = Def.task {
  (client / Compile / fullLinkJS).value
  copyFiles(
    from = (client / Compile / fullLinkJS / scalaJSLinkerOutputDirectory).value,
    to = (Compile / resourceManaged).value / "assets" / "scripts",
...
lazy val copyAssetsTask = Def.task {
  copyFiles(
    from = (client / Compile / resourceDirectory).value,
...
/**
  * Task for copying Scala source files alongside the compiled JS output so that
  * the browser can fetch them when resolving source maps in dev mode. Sources
  * are placed under `assets/scripts/` to match the relative paths embedded in
  * the source map (which are relativised to the project root).
  */
lazy val copySourcesTask = Def.task {
...
/**
  ...
  */
lazy val deleteSourcesTask = Def.task {
  val dest = (Compile / resourceManaged).value / "assets" / "scripts"
...
/**
  ...
  *
  * @param to
  *   The directory to copy files to. The directory structure of the source will
  *   be preserved in the destination.
  *
  * @param cache
  *   A directory to use for caching file hashes, so that only changed files are
  *   copied. By convention, a subdirectory of `streams.value.cacheDirectory`
  *   should be used, although the exact location is not important so long as it
