...
class AssetLoader:
   ...
   *
   * @param path
   *   The path to the asset, represented as a list of "/"-separated components.
   *   The path should be given relative to the resources direction in
  ...
  /**
   * Computes a strong ETag for the given bytes using an MD5 hash. The result is
   * a quoted hexadecimal string, as required by the HTTP specification.
   *
   * @param bytes
   ...
   *   the file extension is unrecognised, returns "application/octet-stream" by
   *   default.
   */
